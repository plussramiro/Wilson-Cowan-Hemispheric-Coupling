...
from src.model import simulate, build_hemispheric_coupling
from src.preprocessing import bessel_filter_bandpass_and_envelope
from src.metrics import calc_corr_dist
from src.plotting import plot_simulation_results_hemispheric


# ========================== Load configs ==========================
hemispheric_params = load_json("hemispheric.json")
...
G1_values = hemispheric_params["G1_values"]  # list of intra-hemispheric coupling values
G2_values = hemispheric_params["G2_values"]  # list of inter-hemispheric coupling values
# ========================================================================
...
results_root = Path("results_hemispheric")
results_root.mkdir(exist_ok=True)   # crea la carpeta si no existe
output_file = results_root / "corr_dist_results.txt"

header = "group,G1,G2,simulation,correlation,distance\n"

if not os.path.exists(output_file):
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(header)

log_file = setup_logging_and_configs(results_root, hemispheric_params, plotting_params, sim_params) # Saving Log and Parameters Used
        ...
        for sim in range(1, num_simuls + 1):
            ...
            for G1 in G1_values:
                for G2 in G2_values:
                    ...
                    CM = build_hemispheric_coupling(SC, G1, G2)    # Build hemispheric coupling matrix: intra scaled by G1, inter scaled by G2
                    ...
                    results.append([g, G1, G2, sim, corr, dist])       

                    if save_figures:
                        plot_simulation_results_hemispheric(time, E_t, E_filt, envelope, SC, FC, FCemp, G1, G2, sim, corr, dist, g, sim_folder, xlim_zoom, ylim_exc_activity, xlim_global, ylim_global)
                    if save_fc_txt:
                        np.savetxt(os.path.join(sim_folder, f"FC_G1_{G1:.2f}_G2_{G2:.2f}_sim{sim}_{g}.txt"), FC, fmt="%.6f", delimiter=",")
                    if save_signals_txt:
                        np.savetxt(os.path.join(sim_folder, f"E_t_G1_{G1:.2f}_G2_{G2:.2f}_sim{sim}_{g}.txt"), E_t, fmt="%.6f", delimiter=",")
                        np.savetxt(os.path.join(sim_folder, f"envelope_G1_{G1:.2f}_G2_{G2:.2f}_sim{sim}_{g}.txt"), envelope, fmt="%.6f", delimiter=",")

                    f.write(f"{g},{G1:.4f},{G2:.4f},{sim},{corr:.6f},{dist:.6f}\n")

                    log_message(f"[INFO] Done hemispheric model with (G1,G2) = ({G1:.2f},{G2:.2f}) for group = {g}, sim = {sim}", log_file)
